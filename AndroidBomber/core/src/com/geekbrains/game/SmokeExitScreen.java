...
/**
 * Created by Irek Nabiullin on 05.04.2018.
 */


public class SmokeExitScreen implements Screen {
    ...
    transient Music musicSmokeExit;
    private Texture background = new Texture(Gdx.files.internal("screensmokingkills.png"));

    // Check music state
    public void checkMusic() {
        if (MusicStatus.INSTANCE.getMusicStatus() == 0) {
            musicSmokeExit.pause();
        }
    ...
    public SmokeExitScreen(SpriteBatch batch) {
        this.batch = batch;
    }
...
//        musicExit = Gdx.audio.newMusic(Gdx.files.internal("musicexit.mp3"));
        musicSmokeExit = Gdx.audio.newMusic(Gdx.files.internal("pilikuril.mp3"));
        musicSmokeExit.setVolume(0.5f);
        musicSmokeExit.setLooping(true);
        musicSmokeExit.play();
    }
...
//        Gdx.gl.glClear(GL20.GL_COLOR_BUFFER_BIT);
        ...
        font96.draw(batch, "Game Over", 250, 500, 1280, 1, false);
        batch.end();
        stage.draw();
    }
...
//        textButtonStyle.checked = skin.getDrawable("transparentButton");
        ...
        btnRestartGame.setPosition(640 - 60 + 260, 250);
        btnExitGame.setPosition(640 - 300 + 290, 250);
